...
class Fcc:
    ...
    def createReport(self, figurePath, docPath, oldallImagesList, title, chapter, isbn, author,
        ...
        print("Length of Fig Dict--->",len(figDict.keys()))
        allImagesList = os.listdir(r"{}".format(figurePath))
        ...
        for j in allImagesList:
            print(j.split(".")[-1],"dpiii")
            try:# j.split(".")[-1]=="eps" and j.split(".")[-1]=="pdf":
                print(figurePath + j, "checkkkkkkkkkk")
        ...
        # Dictionary object called out 
        figDict1 = {}
        k = figDict.keys()
        # print(k)
        for i in k:
            print("Let see for K value-->", k)
            try:
                if re.findall("\.", i):
